...
import com.example.foodorderingapp.navigation.MainAppRoutes
import com.example.foodorderingapp.viewmodel.AuthResult
import com.example.foodorderingapp.viewmodel.AuthViewModel

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun MainScreen(authViewModel: AuthViewModel, mainNavController: NavController) {
    val innerNavController = rememberNavController()
    val navBackStackEntry by innerNavController.currentBackStackEntryAsState()
    val currentRoute = navBackStackEntry?.destination?.route
    val context = LocalContext.current

    // Observe logout state from AuthViewModel
    val authResult by authViewModel.authState.collectAsState()

    LaunchedEffect(authResult) {
        if (authResult is AuthResult.Success && (authResult as AuthResult.Success).hashCode() == "logout".hashCode()) {
            Toast.makeText(context, "Logged out successfully!", Toast.LENGTH_SHORT).show()
            mainNavController.navigate(AppRoutes.LOGIN) {
                popUpTo(AppRoutes.MAIN_APP_CONTAINER) { inclusive = true }
            }
            authViewModel.resetAuthState()
        }
        // Handle logout errors if needed
        if (authResult is AuthResult.Error && (authResult as AuthResult.Error).message == "logout_failed") {
